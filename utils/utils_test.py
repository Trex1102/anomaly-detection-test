...
def compute_pro(masks: ndarray, amaps: ndarray, num_th: int = 200) -> None:
    ...
    assert set(masks.flatten()).issubset({0, 1}), "masks must be binary (0 or 1)"
    assert isinstance(num_th, int), "type(num_th) must be int"

    # Skip images where defect is too small to survive resize (all-zero mask)
    if masks.sum() == 0:
        return 0.0
...
#         df = df.append({"pro": mean(pros), "fpr": fpr, "threshold": th}, ignore_index=True)
    ...
    df = pd.DataFrame(d)
    # Normalize FPR from 0 ~ 1 to 0 ~ 0.05 (MVTec AD 2 standard, AU-PRO_0.05)
    df = df[df["fpr"] < 0.05]
